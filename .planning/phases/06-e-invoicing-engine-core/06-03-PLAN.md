...
must_haves:
  truths:
    ...
    - "QR code data is embedded in cac:AdditionalDocumentReference element"
  artifacts:
    - path: "backend/src/services/einvoice/pint-ae-builder.service.ts"
      provides: "PINT AE XML builder service with QR code embedding"
  ...
  key_links:
    ...
    - from: "pint-ae-builder.service.ts"
      ...
      via: "Embeds QR code from QrCodeService"
      pattern: "AdditionalDocumentReference|EmbeddedDocumentBinaryObject"
---

<objective>
Create the PINT AE XML builder service that transforms Phase 3 TaxInvoiceData into PEPPOL PINT-AE compliant UBL 2.1 XML with embedded QR code.

Purpose: EINV-01 requires generating invoices in PINT AE format (XML structure). This service maps the 13 FTA-mandatory fields from TaxInvoiceData to the corresponding PINT AE XML elements with correct namespaces, structure, and embedded QR code data.
...
<task type="auto">
  <name>Task 1: Create PINT AE Builder Service with QR Code Embedding</name>
 ...
 *
 * QR Code Embedding:
 * Per UBL 2.1, QR codes are embedded in cac:AdditionalDocumentReference
 * with DocumentTypeCode="QR" and the Base64 image in
...
/**
 * QR code document type code per UBL 2.1
 */
const QR_DOCUMENT_TYPE_CODE = 'QR';
...
export class PintAeBuilderService {
   ...
   * @param qrCodeBase64 - Optional Base64-encoded QR code PNG image
   * @returns Complete XML string with declaration
   *
   * @example
   * const xml = pintAeBuilder.buildInvoiceXml(taxInvoice, qrCodeBase64);
   * // Returns: <?xml version="1.0" encoding="UTF-8"?>
   * //          <Invoice xmlns="urn:oasis:names:...">...</Invoice>
   */
  buildInvoiceXml(invoice: TaxInvoiceData, qrCodeBase64?: string): string {
    ...
    const ublInvoice = this.buildInvoiceObject(invoice, qrCodeBase64);
  ...
  buildCreditNoteXml(
    ...
    const ublCreditNote = this.buildCreditNoteObject(creditNote, originalInvoiceNumber, qrCodeBase64);
   ...
   */
  private buildInvoiceObject(invoice: TaxInvoiceData, qrCodeBase64?: string): object {
    return {
      Invoice: {
        ...
        // QR Code embedding (IBG-24: Additional supporting documents)
        ...(qrCodeBase64 && {
  ...
  /**
   * Builds QR code reference structure (IBG-24: Additional supporting documents)
   *
   * Per UBL 2.1 specification:
   * - DocumentTypeCode = "QR"
   * - EmbeddedDocumentBinaryObject contains Base64 PNG
   * - mimeCode = "image/png"
   */
  private buildQrCodeReference(qrCodeBase64: string): object {
    return {
      'cbc:ID': 'QR',
      'cbc:DocumentTypeCode': QR_DOCUMENT_TYPE_CODE,
      'cac:Attachment': {
        'cbc:EmbeddedDocumentBinaryObject': {
          '@_mimeCode': PNG_MIME_TYPE,
...
```
  ...
  <done>PintAeBuilderService created with buildInvoiceXml() and buildCreditNoteXml() methods. Handles all PINT AE mandatory fields with correct namespaces. QR codes embedded in cac:AdditionalDocumentReference per UBL 2.1 spec.</done>
</task>
...
<task type="auto">
  ...
  <action>
Create unit tests for XML structure validation including QR code embedding:
...
describe('PintAeBuilderService', () => {
  ...
  const sampleQrCodeBase64 = 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==';
  ...
  describe('QR Code embedding', () => {
    it('should embed QR code in cac:AdditionalDocumentReference', () => {
      const invoice = createValidInvoice();
      const xml = service.buildInvoiceXml(invoice, sampleQrCodeBase64);

      expect(xml).toContain('<cac:AdditionalDocumentReference>');
      expect(xml).toContain('<cbc:DocumentTypeCode>QR</cbc:DocumentTypeCode>');
    });

    it('should embed QR code with correct MIME type', () => {
      const invoice = createValidInvoice();
      const xml = service.buildInvoiceXml(invoice, sampleQrCodeBase64);

      expect(xml).toContain('mimeCode="image/png"');
      expect(xml).toContain('<cbc:EmbeddedDocumentBinaryObject');
    });

    it('should embed QR code Base64 content', () => {
      const invoice = createValidInvoice();
      const xml = service.buildInvoiceXml(invoice, sampleQrCodeBase64);

      expect(xml).toContain(sampleQrCodeBase64);
    });

    it('should include QR filename attribute', () => {
      const invoice = createValidInvoice();
      const xml = service.buildInvoiceXml(invoice, sampleQrCodeBase64);

      expect(xml).toContain('filename="qrcode.png"');
    });

    it('should NOT include AdditionalDocumentReference when QR code not provided', () => {
      const invoice = createValidInvoice();
      const xml = service.buildInvoiceXml(invoice);

      expect(xml).not.toContain('<cac:AdditionalDocumentReference>');
      expect(xml).not.toContain('<cbc:DocumentTypeCode>QR</cbc:DocumentTypeCode>');
    });

    it('should parse QR code reference correctly', () => {
      const invoice = createValidInvoice();
      const xml = service.buildInvoiceXml(invoice, sampleQrCodeBase64);
      const parsed = parser.parse(xml);

      const docRef = parsed.Invoice['cac:AdditionalDocumentReference'];
      expect(docRef).toBeDefined();
      expect(docRef['cbc:DocumentTypeCode']).toBe('QR');
      expect(docRef['cac:Attachment']).toBeDefined();
    });
  });
  ...
  describe('buildCreditNoteXml', () => {
    ...
    it('should embed QR code in credit note when provided', () => {
      const creditNote = createValidInvoice();
      creditNote.invoiceNumber = 'CN-2026-0001';

      const xml = service.buildCreditNoteXml(creditNote, 'INV-2026-0001', sampleQrCodeBase64);
...
```
  ...
  <done>Unit tests created covering XML structure, namespace declarations, mandatory fields (IBT-xxx), QR code embedding in cac:AdditionalDocumentReference, credit note generation, and XML well-formedness.</done>
</task>
...
5. QR code: Embedded in cac:AdditionalDocumentReference with DocumentTypeCode="QR"
...
6. QR code embedded in cac:AdditionalDocumentReference with EmbeddedDocumentBinaryObject
7. Unit tests cover all mandatory PINT AE fields and QR embedding
</success_criteria>
